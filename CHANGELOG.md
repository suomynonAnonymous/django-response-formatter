...
### Added
- Configurable API prefix detection via `API_PREFIXES` setting (replaces hardcoded `/api/`)
- Throttle/rate-limit response formatting — `Retry-After` header and `retry_after` in metadata for DRF `Throttled` exceptions
- Configurable status code messages via `STATUS_CODE_MESSAGES` setting
- 100% test coverage
